...
export class PostsService{
  ...
  getPost(postsPerPage: number, currentPage: number){
      ...
      this.http.get<{message:string, posts: any, maxPosts: number}>(BACKEND_URL + queryParam)
      .pipe(map((postData) =>{
      ...
      .subscribe((transformedData) =>{
        ...
        this.postCreated.next({posts:[...this.posts], postCount: transformedData.maxPosts});
      })
  }
  ...
  deletePost(paramId: string){
   return this.http.delete(BACKEND_URL +'/'+ paramId);
  }
}
